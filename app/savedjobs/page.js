...
//   }, []);
  ...
  const handleClickClose = (id) => {
    ...
    setSavedJobs(prevJobs =>
      prevJobs.map(job =>
        job.id === id ? { ...job, heartFilled: !job.heartFilled } : job
      )
  ...
  return (
    <>
    <Link href="/swiper" ><VscArrowLeft className={styles.goBack}/></Link>
      ...
      {savedJobs.map(job => (
        <div className={styles.savedJobsList} key={job.id}>
          <div className={styles.imgContainer}>
            <img className={styles.img} src={job.picture} /*alt={imgAlt}*/ height="100px" width="120px"/>
          </div>
          <div className={styles.infoContainer}>
            <div className={styles.innerText}>
              ...
              <div onClick={() => handleClickClose(job.id)} className={styles.close}>{job.heartFilled ? <VscHeartFilled /> : <VscHeart />}</div>
            </div>
